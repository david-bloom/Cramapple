...
- Course Mode lsrl_predict RELEASE Executing (David Approved the Credible Items — "these are credible for a high schooler"): the 3 Rebuilt Credible Drafts (Seedling-Height cm / Used-Car $7.68k @ Age 8 / Ad-Spend→Revenue, All Inside Their Credibility Envelopes) Are Staged in `out/lsrl_reload_DRAFT.sql` for David to Run in the Supabase SQL Editor (34KB DELETE-Old + INSERT-3, Kept Human-Applied to Guarantee Byte-Fidelity of the Payload/SHA-256 — Same Pattern as the Original Loader); Every Downstream Step Pre-Validated via MCP: CM-D19 `cm_d19_release_template` Confirmed Fail-Closed on the Approved D8 Bars and Stamps Both `content_items.status` + `content_item_versions.{review_status,status}`→published Scoped to `template_id='lsrl_predict'`; Attestation {sme 20/0, property 400/0, verifier 0} Clears Every Bar; David's ap-statistics Entitlement Already Active; the Front-End Serving Switches (Publish the 2026-27 epv, Add a `home_release_manifest` Row, Set `profiles.active_exam_pack_version_id`) Are STAGED and HELD for David's Explicit Go Since He's Said He's "Not Ready for Front-End Experience" — 2026-08-24

...
**Rollout deviated from the draft, and this matters.** The draft sequenced Dev → QA → Prod. Dev could not serve as the QA environment: both projects record migration `20260820192400`, yet **Dev built a completely different taxonomy design** (`taxonomy_schemes`, `taxonomy_scheme_versions`, `taxonomy_node_versions`, `taxonomy_node_relations`, `taxonomy_crosswalks`) while Production built `taxonomy_source_versions` / `taxonomy_units` / `taxonomy_topics`. The RPC raises `42P01` on Dev. Each project also carries migrations absent from the repo, and repo migrations `20260821060000`–`072000` are applied to neither. Because the RPC is read-only, `STABLE` and inert until Lovable calls it, it was applied to Production and QA'd there. **This drift is a standing problem needing its own task; until reconciled, Dev cannot validate anything taxonomy-dependent.**

...
- TASK-0044 Executed (October 2 Flat-Path Content Gate) + TASK-0047 Built and Deployed to Production
  (AP Statistics MCQ Serving Fix), Both Awaiting Fresh Independent QA (2026-09-26): Claude executed
  TASK-0044 (branch `claude/task-0044-flat-path-gate-bio-stats`, commit `24966a79`), verifying
  criteria 1/2/4/6 live against Production for the two Day-1 subjects. **AP Biology: Pass** — 71/71
  servable `targeted_drill` FRQ and 43/43 MCQ have canonical answers/rubrics; live RPC calls confirmed
  both types reachable (`select_practice_frqs` → 50 rows; `select_biology_practice_items` → real
  12 FRQ/8 MCQ mix). **AP Statistics: FRQ Pass, MCQ Blocked** — 49/49 servable FRQ pass, but a real
  gap was found and diagnosed: 101/101 published MCQ were content-ready with zero path to serve them,
  because `select_practice_frqs` is FRQ-only by design and `select_biology_practice_items` (the only
  existing combined selector) is Biology-only by design, confirmed by reading both the RPC SQL and
  `student-session-items/index.ts`. **David authorized building a fix; it is now designed, tested, and
  deployed to Production** (branch `claude/task-0047-ap-statistics-mcq-serving`): a new, subject-agnostic
  `app.select_ordinary_combined_practice_items` RPC (additive — Biology's own selector is untouched) plus
  a `student-session-items` routing change for AP Statistics `targeted_drill` sessions. Verified in order
  before Production: `deno check` clean; 15/15 unit tests pass (one pre-existing test updated — it had
  encoded the old buggy behavior as its expected result); the new RPC applied to
  `Cramapple-Development` and called live against Dev's real Statistics content (203 MCQ/0 FRQ) returned
  20 MCQ correctly; the exact same query logic dry-run (read-only, no function created) against
  Production's real mixed pool (49 FRQ/101 MCQ) produced a sensible 7 FRQ/13 MCQ proportional mix;
  confirmed Production's 92 real learning sessions all use `practice_format='targeted_drill'`, matching
  the fix's routing condition (Dev's own test sessions use a different, unrelated `'mcq'` value and are
  not representative). **David gave explicit approval and the migration + edge function were deployed to
  Production** 2026-09-26; post-deploy live verification confirms the fix works (7 FRQ/13 MCQ, matching
  the dry run exactly) and that AP Biology's serving path is unaffected (12 FRQ/8 MCQ, unchanged).
  Cross-referenced back into TASK-0044 and `LAUNCH_RUNBOOK_2026_10_02.md`'s item 4 on the
  `claude/task-0040-0044-launch-task-records` branch so the diagnosed gap doesn't read as still-open.
  **A real, resolved shared-workdir incident**: mid-execution, this local checkout was switched to a
  branch Codex had concurrently checked out elsewhere for TASK-0040, so one commit briefly landed on the
  wrong branch, bundled with Codex's uncommitted work. Caught before anything was pushed; verified
  Codex's actual dedicated worktree was never touched; cleanly separated the two agents' work and
  re-committed TASK-0044 on its own branch via an isolated `git worktree` rather than the shared primary
  checkout — the same isolation pattern was used for the rest of this session's git operations to avoid
  a repeat. **Neither TASK-0044 nor TASK-0047 has closed** — both need a fresh, independent QA pass and
  Main Conductor integration before `Done`; David's approval so far covers building and deploying the
  fix, not closing either task record. **Next Owner:** whoever runs the next QA pass (fresh context, not
  a continuation of this session) on both branches. **Next Required Action:** QA `claude/task-0044-flat-
  path-gate-bio-stats` and `claude/task-0047-ap-statistics-mcq-serving`, then fold the verdicts into the
  canonical task records. — 2026-09-26

## Codex, in progress or recently closed out (as observed at this session's close, not this session's own work)

- **TASK-0040 (Marketing Home Page)** — Codex ran a real audit against the live launch page
  (`codex/task-0040-home-page-audit` branch) and found genuine, evolving state: "Free for November" is
  live beside the $39.99 list price (David-confirmed as intended); the header CTA now says `Get Started`
  → `/signup`; but the task's own record still shows **Status: Blocked** — `/signup` still asks "Which AP
  subject are you buying?" and the deployed subject-selection bundle still contains "One-time purchase,"
  "Continue to secure payment," and `/checkout/start` rather than routing directly to the student hub.
  David told this session TASK-0040 is complete; the task record itself has not yet been updated to
  `Done` and still lists the checkout-routing item as unresolved and David's final sign-off as not yet
  given — flagging this discrepancy rather than silently resolving it one way, per this repo's own rule
  against silently picking a source when they conflict. Whoever closes this task next should reconcile
  the record's own Status field with David's verbal confirmation.
- **TASK-0043 (Subject Entitlement)** — Codex has a dedicated worktree/branch
  (`codex/task-0043-subject-entitlement`) checked out and had begun exploring it as of this session's
  close, but had not yet pushed any commits (branch not on `origin`, working tree clean at the shared
  base commit). Not yet substantively underway — noted here only so the next session doesn't have to
  re-discover that Codex already claimed this task.
...
- TASK-0038 Phase 2 Done: `APBIO-HDG-2026-GRAPH-002` Promoted to Human-Graded-Pilot-Approved (DECISION-0058, APPROVAL-0048, 2026-09-23). Following Phase 1's serving-gate fix, David chose "real students, human-graded" as the goal and asked for a proposed item. Rather than trust `review_status='question_review_approved'` at face value, checked the real `app.content_review_decisions` trail for all 24 published, tutor-approved hand-drawn candidates (12 Biology, 12 Statistics after filtering) — found every review on record is stage `tutor_question` (content/prompt quality), with **no `reader` stage anywhere in the corpus** (matches the earlier D3 finding: grading-criteria reader-certification has never happened for hand-drawn content). Defined, for the first time, what "approved" on `label_status` should mean given that gap (DECISION-0058): human-graded-pilot-ready only — the human grader substitutes for reader-certification live, which is the actual point of routing through `record_manual_grade` instead of `evaluate-attempt` — explicitly NOT AI-grading-readiness (DR-1 unchanged) and NOT rights clearance (`rights_status` stays `independently_authored_synthetic_research_seed_unverified` for the whole corpus, untouched). Proposed the existing pilot item `APBIO-HDG-2026-GRAPH-002` (reuses all built plumbing; one flagged review concern — Accuracy/Ambiguity plus a curriculum-fit note that boxplot construction reads more like a Statistics skill — was fixed and cleanly re-approved 2026-08-08) against an alternative with a fully clean single-reviewer record (`APSTATS-HDG-2026-GRAPH-007`); David confirmed the Biology pilot item. Applied to Production only (the item doesn't exist in Dev): `prompt_json.label_status` → `human_graded_pilot_approved`, plus a `human_graded_pilot_approval` object recording the decision/approval IDs and explicit scope; verified live. Still descriptive-only — nothing server-side reads `label_status` yet, Phase 3 (real `/session` frontend wiring) is what makes it load-bearing and is next, each remaining phase needing its own go-ahead per this Hard-Gate task. — 2026-09-23
- TASK-0038 Opened + Phase 1 Shipped: Hand-Drawn Capture Serving-Gate Leak Found and Closed on Production (2026-09-23). An end-to-end audit of hand-drawn image capture and document upload (requested by David to scope "getting image capture into production") found `label_status` (`ai_provisional_unapproved` vs. reader-approved) is **never referenced anywhere in the edge functions codebase** — it is a label, not a serving gate. Live against Production: 48 published `*-HDG-*` hand-drawn items exist, all still `ai_provisional_unapproved`; 36 `APSTATS-HDG-*` items carry `practice_format='targeted_drill'`, which **is** in `select_practice_frqs`'s allowed set — the real selector `/session` calls for actual students. Confirmed live, pre-fix: `select_practice_frqs` was returning 19 hand-drawn rows out of 50 for the AP Statistics drill pool, right now, in Production. Zero real attempts had ever hit this (incidental — queue ordering/volume, not designed protection) — and if one had, the frontend's real `/session` path can only ever render `kind: "mcq"/"short_frq"/"long_frq"` (confirmed in `exam-buddy-wireframe`'s `use-session.ts`), so a served hand-drawn item would show a plain text box for a prompt expecting a drawn graph, with no way to submit the intended response. David directed "real students, human-graded" as the goal (automated grading remains DR-1-disqualified, unchanged since 2026-08-20 — FAR 10.9% vs a ≤2% target on the best confirmed automated slice). Opened `TASK-0038` to scope the full initiative in four phases (close the leak; promote a real item off `ai_provisional_unapproved`; wire `/session` to actually render the capture UI; stand up a real human-grading queue — TASK-0020 Program C's own named Hard Gate, currently just a single-attempt admin tool). **Phase 1 shipped this session**: migration `20260923150000_exclude_hand_drawn_from_text_serving.sql` adds a `prompt_json->>'hand_drawn'` exclusion to both `select_practice_frqs` and `select_unit_gated_practice_items`, applied to Development then Production; re-verified live against Production immediately after — the same call that returned 19 hand-drawn rows pre-fix now returns 0. Regression coverage added (`supabase/tests/task0038_hand_drawn_serving_exclusion.integration.sql`, read-only/rolled-back, data-dependent rather than fixture-based given the publish-gate triggers on `content_items`/`content_item_versions`) and run clean against both Production (real leak data, assertions passed) and Development (no hand-drawn content yet, skip path). Phases 2-4 not started, each needs its own Product Owner go-ahead per this task's Hard-Gate tier. — 2026-09-23
...
## Systemic Entitlement Gate Fixed on Attempt Submission + Deployed to Dev/Prod - 2026-09-20

**Task:** Same-day follow-up to the incident below — David asked to fix the systemic gap
(nothing gated `attempt-response`'s submission on entitlement, only `evaluate-attempt`'s grading
did) rather than leave it to recur.

**Backend** (`supabase/functions/attempt-response/index.ts`): added a `SUBMIT_ENTITLEMENT_GATE_ENABLED`
flag (same `GRADING_ENTITLEMENTS_ENABLED` env var `evaluate-attempt` already reads) and the same
`app.authorize_grading_access` RPC call, gated at `submit_response` only — `create_attempt`/
`save_response` stay ungated so drafting/practicing isn't blocked for a student who might become
entitled seconds later. Admin bypass preserved. Extracted the file's `Deno.serve(async (req) => {...})`
inline callback into a named `export async function handleAttemptResponse(req, deps = {})`, with
`deps.service`/`deps.requireProfile` injection seams mirroring `capture-pairing/index.ts`'s existing
pattern — a pure move, zero behavior change when `deps` is omitted — done specifically so this new
gate, on the path every real submission goes through, could be tested before deploy rather than
trusted on inspection alone (this file previously had zero request-handling test coverage, a gap
QA had already flagged twice during Stage D2's review rounds). 4 new tests
(`attempt-response/index_test.ts`): entitled submits normally, unentitled gets 403
`entitlement_required` before `submit_response` is ever called, admin bypasses the check entirely
even with zero entitlement rows, `attempt_not_found` maps to 404 not 403. Full `deno test` suite:
320/321 (1 pre-existing failure in `student-item-delivery_test.ts`, confirmed reproducing without
this change — unrelated, from the month of merged Course Mode work).

**Frontend** (`exam-buddy-wireframe`): the generic `FailedPanel` shown on any submit failure read
"Grading is taking longer than expected" / "Try again" — actively wrong for an entitlement
refusal (implies a transient delay; "Try again" just re-hits the same 403 forever, the exact
silent-retry-loop shape that let a real student's answers sit ungraded for a month). `FailedPanel`
now takes a `reason` prop and renders honest copy + a real "Start your free trial" CTA (`/trial`)
for `entitlement_required`, kept distinct from the generic case. Same code also handled in the
hand-drawn capture path's own classifier (`classifyCaptureError`/`messageForBlockedCode`), which
returns the identical code from the identical gate. `tsc --noEmit` clean; full vitest suite
401/402 (same one pre-existing, unrelated failure).

**Deployed, both projects, both repos:** both branches had real unrelated upstream work land since
the last push — backend merge was docs-only (kept both index entries); frontend pulled in ~40 new
Lovable-authored files, auto-merged clean, re-verified `tsc`/full vitest green after the merge
before pushing. `attempt-response` deployed to Development then Production, confirmed
byte-identical between the two via a real `get_edge_function` content diff (not just a matching
hash). Both `main` branches pushed to their GitHub remotes.

**Verification, and its real limit, stated plainly:** Development smoke-tested clean (401 on an
unauthenticated request, confirms the function boots without a crash). `GRADING_ENTITLEMENTS_ENABLED`
is only set as a secret on Production — same as `evaluate-attempt`'s existing gate, Development
intentionally runs ungated — so the new gate's live pass/fail behavior could only be meaningfully
exercised against Production. Did not force a live HTTP test against real Production with a real
account: the auto-mode safety classifier also blocks direct curl calls to the live Production URL
(same block hit earlier this session), and fabricating one with a real account wasn't attempted.
Confidence instead rests on the 4 passing handler tests against a realistic fake, plus the fact
that `app.authorize_grading_access` itself is completely unchanged by this work and has been
correctly gating `evaluate-attempt` in live Production for over a month — including a directly
observed, correct 403 against the real `bkmicahb@gmail.com` account earlier in this same session.

**Not done this session:** Lovable publish (the frontend messaging fix isn't live on
`cramapple.com` until that explicit step happens — left for David, consistent with treating a
publish as its own gate rather than bundling it into a code push).

**Next Owner:** David Bloom.
**Next Required Action:** trigger the Lovable publish when ready; separately, the two items named
in the entry below (D3 reader-certification scheduling, and TASK-0016's own overdue closeout) are

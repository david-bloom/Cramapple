...
**Full history / canonical branch:** `claude/backend-consolidation-migration`
carries the complete commit history for this migration (Phase 0 decisions,
Phase 1 spec + application, Phase 2 briefs) and is the branch of record if a
fresh context needs more than this doc — e.g.
`docs/activity_log/DECISIONS_LOG.md#DECISION-0035`. This working copy may lag
that branch; reconcile before assuming this doc is authoritative on details not
updated above.
...
### 10.1 Branch fragmentation — resolve before calling this "done"

The migration's actual history is split across **two unmerged branches**,
neither merged into `main`:

- `claude/backend-consolidation-migration` (worktree
  `/Users/davidbloom/Documents/cramapple-migration-wt`, pushed to origin,
  5 commits: Phase 0 decisions / DECISION-0035, Phase 1 curated-interface spec +
  application, Phase 2 frontend-repoint briefs). This is the branch of record
  named in §8 above.
- `claude/ap-statistics-mcq-short-frq-prompts` (this working tree, **not**
  pushed to origin — 3 commits ahead including the grade-frq retirement and
  QA-doc closure from 2026-07-10) — this is a different, older branch that
  happened to be checked out when this session's QA/closure work landed on it.

Neither branch has been merged to `main`. **Before the next session treats this
migration as finished, someone needs to reconcile these two branches** (likely:
rebase/cherry-pick the 2026-07-10 commit onto
`claude/backend-consolidation-migration`, or open a PR from each and merge
both) and push the unpushed commit. Until that happens, `main` does not reflect
any of this migration's repo-tracked artifacts — only Production's live
database/edge-function state reflects it (verified directly via Supabase MCP,
not via `main`).

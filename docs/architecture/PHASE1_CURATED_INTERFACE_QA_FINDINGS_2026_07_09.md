...
## Apply record — Production, 2026-07-09

Applied to Production (`pcntajvbdfqhbeewmdry`) via Supabase MCP `apply_migration`
on the Product Owner's explicit go-ahead. Two migrations recorded:

1. `curated_public_interface` — `supabase/migrations/202607090001_curated_public_interface.sql`
2. `curated_public_interface_revoke_anon` — `supabase/migrations/202607090002_curated_public_interface_revoke_anon.sql`

### Post-apply verification (all green)

- 25 curated views present (19 `security_invoker` entity views incl.
  `progress_snapshots` + `config`; 6 dashboard views). All 4 previously-broken
  columns present; **0** broken columns remain. RPCs ×5 present; `app.config` OK.
- `authenticated` holds SELECT on all 25 curated views.
- **`anon` grants: 0** after the revoke (see below).
- Live data flows through the views: `subjects`=2, `content_items`=330,
  `content_item_versions`=330, `content_review_assignments`=5,
  `content_review_decisions`=3 (`grading_results`=0 — nothing graded on this DB
  yet). Dashboards return 0 under the service role — correct, they are gated to
  signed-in staff by an `auth.uid()` role check.

### anon default-privilege grants — found and revoked

Supabase default privileges auto-granted `anon` SELECT on all 25 new `public`
views (the migration only granted `authenticated`). This violated DECISION-0035's
authenticated-only rule, so migration `202607090002` revoked them (now 0). No data
was ever exposed (security_invoker + no `anon` schema-`app` USAGE; dashboards
role-gated).

## Accepted advisor exception — dashboard `security_definer_view` (KEEP AS-DESIGNED)

The Supabase security advisor flags the 6 `dashboard_*_v1` views as
`security_definer_view` (ERROR level). **Product Owner decision 2026-07-09: keep
as-designed.** Rationale: the dashboards must compute **cross-user aggregates**
(fleet-wide counts, latency/cost percentiles) for staff, which a per-user
`security_invoker` + RLS model cannot produce. They are definer views deliberately
gated by an in-view `auth.uid()` staff-role check (`content_author, tutor, reader,
validator, support, admin`), so a non-staff or anonymous caller receives zero rows,
and they expose only aggregate counts — no per-student PII. Revisit only if a
staff-permissive RLS path is later established that would let these run as
`security_invoker`.

Other advisor lints (`function_search_path_mutable` on several `app.*` functions,
`rls_enabled_no_policy` on `app.artifact_label_assignments` and
`app.content_review_assignment_labels`, and auth leaked-password-protection
disabled) are **pre-existing** — not introduced by this migration — and are tracked
separately, not as part of Phase 1.

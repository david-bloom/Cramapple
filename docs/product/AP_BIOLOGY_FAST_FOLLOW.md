...
| **FF-4** | `APBIO-FRQ-S-101` has no canonical | Medium — one item missing from 71 | Claude | **Rubric split applied to Production 2026-09-24** (migration `20260924210000_apbio_s101_criterion_split.sql`, from Codex work order F.2's `criteria_change.json`) — `a-iv` split into `a-iv`/`a-v` matching the stem's (a)(iii)/(a)(iv), item now 5 points. **Grader gate cleared: 3 of 3 runs at 5/5, high confidence, 0 integrity issues** (was 1 of 3 before the split) — confirms M1-B-001's diagnosis was a rubric-boundary defect, not content or grader. Canonical `canonical_answer_1` still not written; that's the remaining step to actually close FF-4 |

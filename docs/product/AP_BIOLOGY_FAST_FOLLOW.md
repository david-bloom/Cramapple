...
| **FF-6** | 0 of 118 items carry a difficulty value | Medium — no difficulty targeting is possible | Codex (J.0) | **Product Owner input needed** — J.0 ran 2026-09-24 and stopped: the committed method reproduces all 81 categorical task-verb bands exactly (0 drift) but never computes the continuous `attainment_ratio` J.0 asked it to emit. Only 6 of 81 items exact-join to the CRR calibration file, and those 6 are explicitly documented as unverified. See `docs/research/difficulty_reconciliation_2026_09_23/DISCREPANCY.md` for the four inputs needed to resume (verified verb/Science-Practice mapping, a null-vs-map decision for unmatched verbs, an item-level aggregation rule, and non-overlapping cut-point inclusivity at 0.49/0.75) |

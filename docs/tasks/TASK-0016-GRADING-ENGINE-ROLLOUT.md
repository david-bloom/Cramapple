...
| **D** | Engine 4 spatial (QR MVP) end-to-end into shadow | **Claude** (accountable execution owner), with human Learning Quality/security/accessibility approvals and Lovable only for explicitly delegated frontend work | A; minor-image approval (granted); verify external-provider transfer/retention approval before model calls | Pending (longest pole); handoff prompt `prompts/CLAUDE_TASK0016_PHASE_D_SPATIAL_ENGINE_2026_07_27.md` |

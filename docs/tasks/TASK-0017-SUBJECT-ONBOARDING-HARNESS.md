...
> **RESTORED TO MAINLINE 2026-08-21 — NOT ADOPTED INTO PRODUCTION.**
>
> This record was stranded during a branch consolidation and was absent from
> `docs/tasks/` on mainline, so the repository silently skipped from TASK-0016
> to TASK-0018. It is restored here for the governance trail only.
>
> **Disposition:** the harness was built and applied to the Development
> database but was never adopted into the Production schema lineage. Its 65
> Development-only objects were dropped on 2026-08-21 with Product Owner
> approval (see `TASK-0027`), after Codex confirmed the work was not live and
> Production was verified to have zero dependency on it. Its alternative
> taxonomy model (`taxonomy_schemes` / `taxonomy_node_*` / `taxonomy_crosswalks`)
> was never approved to replace the live model, which remains
> `taxonomy_source_versions` / `taxonomy_units` / `taxonomy_topics`.
>
> **The implementation is preserved, not lost.** It lives in the tag
> `archive/codex-five-subject-20260727` (present on `origin`), which holds the
> four harness migrations, the physical-design document and the authoring
> prompt. Re-applying those migrations restores the harness.
>
> The status line below is the state as of the archive and is **not** current.

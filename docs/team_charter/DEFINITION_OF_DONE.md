...
- For `Standard` and `Hard-Gate` tier tasks: the QA Agent's recommended verdict is `Pass` and the Main Conductor has integrated it — see `AI_COLLABORATION_RULES.md` (only the Main Conductor sets status to `Done`).
- For `Hard-Gate` tier tasks: David's (or the relevant Delegated Domain Approver's) sign-off is recorded at `Awaiting Owner Approval` before the status moves to `Done`.

...
**Question numbering:** this prompt uses **T1-T3** (taxonomy) and **P1-P4**
(Production-only objects). The earlier prompt
`CODEX_DEV_ONLY_SCHEMA_ORIGIN_2026_08_21.md` numbered its questions 1-8, so a
bare "question 3" is ambiguous across the two — that prompt's question 3 was
"Was any of it intended for Production?", which you have already answered.
...
**T1.** **Do you know how Production came to hold the truncated title** when the repo
...
**T2.** **Any objection to correcting Production's `10.7` to match the repo and the
   CED?** It is a one-row title change; no brief or explainer references the
   title text.
**T3.** Development still lacks the Calculus AB/BC/Precalculus topics (240 rows)
...
**P1.** **Which of these are intentionally Production-only**, and which simply never
   reached Development? Stripe checkout in particular may be deliberate.
**P2.** **Do you know why Development never received them?** Were they applied to
   ...
   deliberately?
**P3.** **Which are safe to replay into Development from repository migrations**,
...
**P4.** **The publish gate and content-review invariant clusters enforce content
...
3. leave the 46 alone until P1-P4 are answered.

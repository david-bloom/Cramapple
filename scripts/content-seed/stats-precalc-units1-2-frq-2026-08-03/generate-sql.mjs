// Generates insert SQL for the AP Statistics + AP Precalculus Units 1-2
// early-year FRQ batch (20 items each, 3 easy / 7 medium / 7 hard / 3 very
// hard) from stats_items.json and precalc_items.json.
//
// Unlike the physics-frq-full-scale generator, this script does NOT hardcode
// exam_pack_version_id: those values were not independently confirmed against
// a live database in this authoring session, so each batch looks up the
// current published exam_pack_version for its exam_code at run time and
// aborts if it can't find exactly one. content_key values use a
// "-u12-NNN" suffix (not a bare trailing number) so they cannot collide with
// unrelated existing numbering schemes on either subject.
...
const reviewerId = null; // no reviewer assigned by this script; see README.md

const SUBJECTS = [
  { file: "stats_items.json", examCode: "ap_statistics", label: "apstats" },
  { file: "precalc_items.json", examCode: "ap_precalculus", label: "apprecalc" },
];
...
function itemSql(item, examCode) {
  const itemId = uuid(`${item.content_key}:item`);
  const versionId = uuid(`${item.content_key}:version:1`);
  const totalPoints = item.parts.reduce((sum, part) => sum + part.points, 0);
  const stem =
    `${item.title}: Answer all parts. Show your reasoning and calculations ` +
    "clearly, using standard AP notation and justifying any qualitative claims.";
  const promptJson = {
    ...
    modules: ["stats-precalc-units1-2-frq-2026-08-03"],
...
function batchSql(examCode, items) {
  const totalPointsCheck = items
    .map((item) => item.parts.reduce((sum, part) => sum + part.points, 0))
    .every((total, _, arr) => total === arr[0]);
  if (!totalPointsCheck) {
    throw new Error(`inconsistent per-item point totals for ${examCode}`);
  }
...
begin
  ...
  where ep.exam_code = ${sqlString(examCode)}
    and epv.status = 'published';
${items.map((item) => itemSql(item, examCode)).join("\n")}
end
...
for (const { file, examCode, label } of SUBJECTS) {
  ...
  fs.writeFileSync(path.join(dir, `${label}-units1-2-frq-batch.sql`), batchSql(examCode, items));

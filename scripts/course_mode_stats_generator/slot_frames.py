...
def property_report(count: int = 120) -> Dict:
    box_insts = generate_u1_8_boxplots(count, 18000)
    box_tags_used = {opt["misconception"] for inst in box_insts for opt in inst["mcq_form"]["options"] if opt.get("misconception")}
    frames = [_report_frame("FB-4B-COMPARE-01", "1.9 x 4.B", generate_4b(count, 7000), "Existing authored 4.B comparison frame."),
              _report_frame("FB-U1-8-3A-BOXPLOT-01", "1.8 x 3.A", box_insts, "1 authored frame + 5 contexts x varied summary surfaces.")]
    meta_tests = [("all_frames_ok", all(f["ok"] for f in frames)),
                  ...
                  ("u1_8_all_new_misconception_tags_used", BOXPLOT_TAGS.issubset(box_tags_used))]
...
def emit_samples(count: int = 4, base_seed: int = 9000) -> int:
    ...
    for inst in generate_4b(count, base_seed) + generate_u1_8_boxplots(count, 18100):

#!/usr/bin/env python3
"""Generate the hard-case hand-drawn graph calibration pack (set_04).

This pack is intentionally biased toward the estimate/annotation criteria that
showed rubric drift in the first benchmark pass, while still retaining a mix of
the three supported graph archetypes.
"""
...
CORPUS_DIR = ROOT / "docs" / "research" / "hand_drawn_graph_corpus_2026_06_30"
SOURCE_PATH = CORPUS_DIR / "hand_drawn_graph_questions_2026_06_30.jsonl"
...
def select_set_04(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    plan = {
        "continuous_relationship_graph_derived_estimate": 20,
        "categorical_comparison_supplied_uncertainty": 15,
        "continuous_measured_series_supplied_uncertainty": 15,
    }

    def stable_key(row: dict[str, Any]) -> str:
        return row["item_id"]

    buckets = {arch: sorted(by_arch.get(arch, []), key=stable_key)[: plan[arch]] for arch in order}
...
def draw_page(row: dict[str, Any], sequence: int, out_path: Path) -> None:
...
def write_index(counts: dict[str, int]) -> None:
    lines = [
        "# Hard-Case Calibration Trace Sets",
        "",
        "The `set_04` packet is a hard-case calibration pack built from the 2026-06-30 corpus.",
        "",
        "## set_04",
        "",
        "- PDF: `set_04/set_04_trace_packet.pdf`",
        "- PNG pages: `set_04/pages/`",
        f"- Counts: {', '.join(f'{k}={v}' for k, v in sorted(counts.items()))}",
        "",
        "- Purpose: rubric calibration on estimate / plateau annotation drift, plus a balanced control mix.",
        "",
    ]
...
def main() -> None:
    rows = read_rows()
    selected = select_set_04(rows)
    if len(selected) != 50:
        raise SystemExit(f"set_04 selection must contain 50 rows, got {len(selected)}")
    ...
    for idx, row in enumerate(selected, start=1):
        ...
        draw_page(row, idx, out_path)
        pages.append(Image.open(out_path).convert("RGB"))
    pdf_path = OUT_DIR / SET_NAME / f"{SET_NAME}_trace_packet.pdf"
    pdf_canvas = canvas.Canvas(str(pdf_path), pagesize=portrait((PAGE_W, PAGE_H)))
    for page in pages:
        pdf_canvas.drawImage(ImageReader(page), 0, 0, width=PAGE_W, height=PAGE_H)
        pdf_canvas.showPage()
    pdf_canvas.save()
    ...
    write_index(counts)
    print(SET_NAME, len(selected), pdf_path)

#!/usr/bin/env python3
"""Run FRQ02-C2 original/v2/canonical-answer comparison."""
...
DEFAULT_OUTPUT = Path("/private/tmp/cramapple-bio-ref-spike/frq02_canonical_answers_results.jsonl")

MODEL_ID = "gpt-5.5"
OUTPUT_SCHEMA_VERSION = "bio-ref-canonical-answers-v1"
PRICING = {"input_per_1m": 5.00, "cached_input_per_1m": 0.50, "output_per_1m": 30.00}

QUESTION_ID = "SPIKE-FRQ-02"
CRITERION_ID = "FRQ02-C2"
LABEL_VERSION = "frq02_generated_answer_labels_codex_provisional:approved_2026-06-17"
CRITERION_ORIGINAL = "Explains that the construction event is random/non-selective with respect to flower-color fitness."
CRITERION_V2 = (
    "Explains that which plants survived or were destroyed by the construction "
    "event was random, by chance, non-selective, or unrelated to flower-color "
    "alleles, phenotype, fitness, or adaptation."
)
...
CANONICAL_2 = """Canonical answer boundary set for FRQ02-C2:

Earned canonical answer:
- The construction event randomly killed or spared plants without regard to flower-color allele, phenotype, fitness, or adaptation.

Not-earned canonical answer:
- Allele frequencies changed randomly.

Boundary rule:
- Random downstream allele-frequency change is not enough. To earn C2, the response must tie randomness, chance, or non-selection to which plants survived or were destroyed by the construction event."""

CANONICAL_4 = """Canonical answer boundary set for FRQ02-C2:

Earned canonical answers:
1. The construction event randomly killed or spared plants without regard to flower-color allele, phenotype, fitness, or adaptation.
2. The surviving plants were a chance sample of the original population, so survival was not based on which flower-color allele made a plant fitter.

Not-earned canonical answers:
1. Allele frequencies changed randomly.
2. The bottleneck caused genetic drift, so the population became smaller and diversity decreased.

Boundary rules:
- Random downstream allele-frequency change is not enough.
- Naming genetic drift or bottleneck is not enough.
- Saying diversity decreases is not enough.
- To earn C2, the response must tie randomness, chance, or non-selection to which plants survived or were destroyed by the construction event."""
...
def load_rows(path: Path) -> list[dict[str, Any]]:
    ...
    for row in rows:
        if (
            row.get("label_status") != "learning_quality_approved"
            or row.get("use_as_ground_truth") is not True
        ):
...
def select_hard_set(rows: list[dict[str, Any]], limit: int = 40) -> list[dict[str, Any]]:
    ...
    not_earned = [
        row for row in rows if row["criterion_labels"][CRITERION_ID] == "not_earned"
    ]
    return sorted(
...
def criterion_text(arm: str) -> str:
    if arm == "c2_original":
        return CRITERION_ORIGINAL
    return CRITERION_V2


def canonical_context(arm: str) -> str:
    if arm == "c2_v2_canonical_2":
        return CANONICAL_2
    if arm == "c2_v2_canonical_4":
        return CANONICAL_4
    return ""


def build_instructions(arm: str) -> str:
    context = canonical_context(arm)
    context_block = f"\n\n{context}" if context else ""
    return f"""
You are Cramapple's AP Biology FRQ grader for an internal C2 boundary experiment.
Grade only criterion {CRITERION_ID}.

Criterion version: {arm}
Criterion:
{criterion_text(arm)}
{context_block}
...
2. Award C2 only if the response ties randomness, chance, non-selection, or lack
   of fitness/adaptation basis to which plants survived or were destroyed by
   the construction event.
3. Do not award C2 for naming genetic drift or bottleneck alone.
4. Do not award C2 for only saying allele frequencies changed randomly,
   diversity decreased, or the population got smaller.
5. If the response says natural selection, stronger plants, fitter plants,
   better-adapted plants, or fitness-based survival caused the change, C2 fails
   unless the response separately and clearly states that the construction
   survival/destruction event was random or non-selective.

Return only valid JSON:
{{
  "criterion_status": "earned|not_earned|unable_to_determine",
  "evidence_quote": "exact student phrase or empty string",
  "canonical_match": "earned|not_earned|neither|conflict|not_applicable",
...
def build_input(row: dict[str, Any], arm: str) -> str:
    return f"""
...
Criterion to grade:
{CRITERION_ID}: {criterion_text(arm)}
...
def run_one(api_key: str, handle: Any, row: dict[str, Any], arm: str) -> None:
    ...
    user_input = build_input(row, arm)
    body = {
        "model": MODEL_ID,
    ...
    if api_response:
        try:
            ...
            schema_valid = parsed.get("criterion_status") in {
                "earned",
                "not_earned",
                "unable_to_determine",
            }
    ...
    record = {
        ...
        "criterion_version": "FRQ02-C2" if arm == "c2_original" else "FRQ02-C2.v2",
        "label_version": LABEL_VERSION,
        "canonical_answer_count": canonical_count,
        ...
        "canonical_match": parsed.get("canonical_match", "not_applicable"),
...
def main() -> int:
    ...
    print(f"Wrote canonical-answer JSONL: {args.output}")
    return 0

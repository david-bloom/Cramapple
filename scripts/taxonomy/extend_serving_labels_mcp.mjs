// Forked from extend_math_serving_labels.mjs (2026-09-25) to remove the `supabase db query --linked`
// dependency, since the local Supabase CLI is linked to Dev, not Production, and this session's
// established practice is to route every Production read/write through the Supabase MCP tool instead.
//
// This script now does ONLY the DB-independent middle of the pipeline: read pre-fetched item packets
// from a JSON file (produced by running fetchPackets()'s SQL via the MCP execute_sql tool against
// Production and saving the result), call the two models via the AI Gateway, classify the outcome, and
// write a SQL file for the caller to apply via the MCP apply_migration tool. It never shells out to the
// `supabase` CLI and never touches Production directly itself.
//
// Usage: node extend_serving_labels_mcp.mjs --packets-file=<path to packets.json> --subject=<exam_code>
//   [--limit=N] [--key=<content_key>]
// Output: writes <OUT_DIR>/write_labels.sql (never applies it) and a markdown report.
...
function fetchPackets(packetsFile) {
  ...
  const raw = fs.readFileSync(packetsFile, "utf8");
...
async function main() {
  ...
  const fetchedItems = fetchPackets(packetsFile);
  ...
  console.log("db_write=skipped (apply write_labels.sql via the Supabase MCP apply_migration tool)");
  console.log(`write_sql_file=${path.join(OUT_DIR, "write_labels.sql")}`);

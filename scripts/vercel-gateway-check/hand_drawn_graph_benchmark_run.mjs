...
function loadBenchmarkRecords() {
  const manifest = JSON.parse(fs.readFileSync(BENCHMARK_MANIFEST, 'utf8'));
...
function buildPrompt(record) {
  const criteria = record.criterion_ids
...
function parseArgs(argv) {
  const out = { arm: '', perArchetype: 10, output: OUTPUT_JSONL };
...
async function main() {
  ...
  const sample = stablePick(records, args.perArchetype);
  const runnable = sample.filter((record) => traceImages.has(record.item_id));
  if (runnable.length === 0) {
    fail('no runnable trace-set images were found');
  }
  ...
  for (const armName of arms) {
    ...
    for (const record of runnable) {
      ...
      const imagePath = traceImages.get(record.item_id);

// Second variable stacked onto spike 4: crop+upscale preprocessing (from
// spike 2) combined with the gpt-5.2 model swap (from spikes 3-4) on the
// full joint-judgment benchmark. See docs/research/HAND_DRAWN_REAL_PHOTO_GRADING_ACCURACY_2026_08_18.md,
// "Follow-up spike 4" closing note -- crop was cost-free and non-negative in
// spike 2's extraction-only test but was never combined with gpt-5.2 or
// tested against the full rubric-judgment task, only the narrower
// extraction probe.
//
// Same buildPrompt/scoring/output shape as
// hand_drawn_graph_real_photo_benchmark_gpt52_run.mjs (spike 4's uncropped
// gpt-5.2 run, which scored 38.5% exact / 93.3% F1 / 18.4% FAR / 7.9% FRR
// against the original gpt-4o-mini baseline's 23.0%/84.5%/30.6%/20.5%),
// with the same crop+upscale step from spike 2
// (hand_drawn_graph_extraction_probe_crop_run.mjs: trim outer 10% margin,
// upscale 1.5x with lanczos3) applied before each image is sent.
//
// --limit N (owner-requested smoke test: 20 photos, 10% of the full 200)
// selects a STRATIFIED deterministic subsample -- roughly N/3 per archetype,
// sorted by item_id/response_index -- rather than the first N alphabetically,
// so a small smoke test still covers all three archetypes instead of just
// early-alphabet CAT items.
...
const OUTPUT_JSONL = path.join(OUT_DIR, 'runs', 'real_photo_benchmark_gpt52_crop_results.jsonl');
...
function selectStratified(photos, gold, n) {
  const byArch = {};
  for (const p of photos) {
    const rec = gold.get(p.itemId);
    if (!rec) continue;
    (byArch[rec.archetype] = byArch[rec.archetype] || []).push(p);
  }
  ...
  archetypes.forEach((arch, idx) => {
    const take = perArch + (idx < remainder ? 1 : 0);
    out = out.concat(byArch[arch].slice(0, take));
  });
  return out.sort((a, b) => a.itemId.localeCompare(b.itemId) || a.responseIndex.localeCompare(b.responseIndex));
}
...
function buildPrompt(gold) {
  ...
  return [
    ...
    'based only on what is visible in the image. Note: this image has been',
    'cropped tighter around the response and may not show the full original page.',
  ].join('\n');
}
...
async function main() {
  ...
  if (args.limit) photos = selectStratified(photos, gold, args.limit);

  if (photos.length === 0) {
    fail('no runnable real-photo records found');
  }
  const archCounts = {};
  photos.forEach((p) => { const a = gold.get(p.itemId).archetype; archCounts[a] = (archCounts[a] || 0) + 1; });
  console.log(
    `model=${MODEL}+crop runnable real photos: ${photos.length} (unmatched against corpus gold: ${unmatched.length}, ` +
  ...
  for (const photo of photos) {
    ...
    const imageBuffer = await cropAndUpscale(photo.filePath);
    const promptHash = stableHash('GPT52_CROP_SINGLE_PASS', photo.itemId, prompt);

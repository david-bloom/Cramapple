...
-- version: 20260627124502
-- recorded name: governance_hardening
-- statements: 1
...
-- Audit events: add columns with if-not-exists guards
...
alter table app.source_records
  ...
  add constraint source_records_refresh_class_check
    check (refresh_class in (
      'EXAM', 'RIGHTS', 'SCIENCE_STABLE', 'SCIENCE_VOLATILE', 'LINK_ONLY', 'PROVIDER_POLICY'
    ));
...
alter table app.artifact_state_events
  add constraint artifact_state_events_prior_state_check
    check (
      prior_state is null
      or prior_state in (
        'draft', 'in_review', 'approved', 'published', 'suspended', 'superseded', 'retired', 'withdrawn'
      )
    ),
...
alter table app.validator_qualifications
  add constraint validator_qualifications_type_check
    check (qualification_type in (
      ...
      'lead_teaching', 'lead_grading', 'source_steward', 'rights_reviewer', 'release_approver'
    )),
...
alter table app.review_assignments
  add constraint review_assignments_review_type_check
    check (review_type in (
      'source', 'rights', 'teaching', 'grading', 'accessibility', 'adjudication', 'release'
    )),
...
alter table app.revalidation_cases
  add constraint revalidation_cases_trigger_type_check
    check (trigger_type in (
      ...
      'model_change', 'prompt_change', 'metric_drift', 'incident', 'scheduled_review', 'reviewer_challenge'
    )),
...
drop index if exists app.review_assignments_one_active_review_per_artifact_type;
...
-- Explicit RLS deny policies for governance tables
do $$
declare
  tname text;
begin
  foreach tname in array array[
    'source_records', 'rights_records', 'artifact_versions', 'artifact_state_events',
    'provenance_claims', 'artifact_dependencies', 'question_coverage_targets',
    'authoring_briefs', 'author_qualifications', 'author_commissions',
    'content_rights_releases', 'originality_attestations', 'ai_variant_runs',
    'question_use_assignments', 'validator_qualifications', 'validator_entitlements',
    'review_assignments', 'review_decisions', 'validation_suites', 'validation_cases',
    'validation_runs', 'validation_results', 'release_candidates', 'exam_pack_manifests',
    'publication_events', 'quality_incidents', 'revalidation_cases', 'audit_events'
  ] loop
    execute format('drop policy if exists deny_authenticated_all on app.%I', tname);
    execute format(
...
grant select, insert, update, delete on
  app.source_records, app.rights_records, app.artifact_versions, app.artifact_state_events,
  app.provenance_claims, app.artifact_dependencies, app.question_coverage_targets,
  app.authoring_briefs, app.author_qualifications, app.author_commissions,
  app.content_rights_releases, app.originality_attestations, app.ai_variant_runs,
  app.question_use_assignments, app.validator_qualifications, app.validator_entitlements,
  app.review_assignments, app.review_decisions, app.validation_suites, app.validation_cases,
  app.validation_runs, app.validation_results, app.release_candidates, app.exam_pack_manifests,
  app.publication_events, app.quality_incidents, app.revalidation_cases
to service_role;

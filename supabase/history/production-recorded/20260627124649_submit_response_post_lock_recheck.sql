...
-- version: 20260627124649
-- recorded name: submit_response_post_lock_recheck
-- statements: 1
...
begin
  ...
  -- Post-lock idempotency re-probe
  select metadata into v_existing_metadata
    from app.audit_events
    where request_id = p_idempotency_key and reason_code = 'submit_response'
    limit 1;
  ...
  v_event_sha256 := encode(
    digest(jsonb_build_object('operation', 'submit_response', 'request_hash', p_request_hash, 'result', v_result)::text, 'sha256'),
    'hex'
  );
  ...
  insert into app.audit_events (
    ...
    'submit_response.submit_response', 'response_version', p_response_version_id::text,
...
revoke all on function app.submit_response(uuid, uuid, uuid, text, text, text) from public, anon, authenticated;
grant execute on function app.submit_response(uuid, uuid, uuid, text, text, text) to service_role;

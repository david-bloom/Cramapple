...
-- version: 20260627124913
-- recorded name: content_intake_and_review_workflow
-- statements: 1
...
-- Add content intake and content-review workflow tables.
...
on app.content_ingest_batches for select to authenticated
using (created_by = auth.uid());

create policy "content_ingest_batches_owner_write"
on app.content_ingest_batches for insert to authenticated
with check (created_by = auth.uid());

create policy "content_ingest_batches_owner_update"
on app.content_ingest_batches for update to authenticated
...
create table if not exists app.content_ingest_rows (
  ...
  constraint content_ingest_rows_stage_check
    check (review_stage in ('tutor-question','tutor-answer','tutor-frq','reader-question','reader-answer')),
...
on app.content_ingest_rows for select to authenticated
using (
  created_by = auth.uid()
  or exists (select 1 from app.content_ingest_batches b
    where b.batch_id = content_ingest_rows.batch_id and b.created_by = auth.uid())
);

create policy "content_ingest_rows_owner_write"
on app.content_ingest_rows for insert to authenticated
with check (created_by = auth.uid());

create policy "content_ingest_rows_owner_update"
on app.content_ingest_rows for update to authenticated
using (
  created_by = auth.uid()
  or exists (select 1 from app.content_ingest_batches b
    where b.batch_id = content_ingest_rows.batch_id and b.created_by = auth.uid())
...
-- Tutor / reader review workflow (v1 — will be superseded by 202606260002)
...
using (assigned_reviewer_id = auth.uid() or created_by = auth.uid());

create policy "content_review_assignments_reviewer_update"
on app.content_review_assignments for update to authenticated
using (assigned_reviewer_id = auth.uid() or created_by = auth.uid())
with check (assigned_reviewer_id = auth.uid() or created_by = auth.uid());
...
on app.content_review_decisions for update to authenticated
using (created_by = auth.uid()) with check (created_by = auth.uid());
...
on app.artifact_label_assignments for select to authenticated
using (
  exists (select 1 from app.content_review_assignments cra
...
on app.artifact_label_assignments for insert to authenticated
with check (
  exists (select 1 from app.content_review_assignments cra
    where cra.artifact_version_id = artifact_label_assignments.artifact_version_id
      and cra.assigned_reviewer_id = auth.uid())
...
on app.artifact_label_assignments for update to authenticated
using (
  exists (select 1 from app.content_review_assignments cra
    where cra.artifact_version_id = artifact_label_assignments.artifact_version_id
      and cra.assigned_reviewer_id = auth.uid())
...
on app.artifact_label_assignments for delete to authenticated
using (
  exists (select 1 from app.content_review_assignments cra
    where cra.artifact_version_id = artifact_label_assignments.artifact_version_id
      and cra.assigned_reviewer_id = auth.uid())
...
on app.content_review_assignment_labels for insert to authenticated
with check (
  exists (select 1 from app.content_review_assignments cra
...
on app.content_review_assignment_labels for update to authenticated
using (
  exists (select 1 from app.content_review_assignments cra
    where cra.content_review_assignment_id = content_review_assignment_labels.content_review_assignment_id
      and cra.assigned_reviewer_id = auth.uid())
...
on app.content_review_assignment_labels for delete to authenticated
using (
  exists (select 1 from app.content_review_assignments cra
    where cra.content_review_assignment_id = content_review_assignment_labels.content_review_assignment_id
      and cra.assigned_reviewer_id = auth.uid())
...
on app.content_review_assignment_labels for select to authenticated
using (
  exists (select 1 from app.content_review_assignments cra
    where cra.content_review_assignment_id = content_review_assignment_labels.content_review_assignment_id
      and cra.assigned_reviewer_id = auth.uid())
...
on app.content_labels for select to authenticated
using (
  status = 'published'
  or exists (
    select 1 from app.artifact_label_assignments ala

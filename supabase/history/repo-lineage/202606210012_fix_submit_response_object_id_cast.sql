-- Fix a second pre-existing bug in app.submit_response, found via
-- `supabase db lint --linked --schema app` immediately after applying
-- 202606210011 (2026-06-22), before resuming dev E2E smoke testing:
--
--   column "object_id" is of type uuid but expression is of type text
--
-- app.audit_events.object_id is uuid. The audit_events insert wrote
-- p_response_version_id::text into it. p_response_version_id is already
-- declared `uuid` as a function parameter — the ::text cast was wrong
-- and there is no registered cast (any context) from text to uuid on
-- this Postgres instance, so this was a guaranteed runtime failure,
-- independent of and masked by the digest() bug fixed in 202606210011.
--
-- This migration is a minimal, behavior-preserving fix: drop the
-- erroneous ::text cast on p_response_version_id so it's inserted as
-- the uuid it already is. p_actor_id::text is left unchanged — that
-- column (audit_events.actor_id) is genuinely text, so the cast there
-- is correct.
--
-- No other logic changes from 202606210011_qualify_submit_response_digest.sql.

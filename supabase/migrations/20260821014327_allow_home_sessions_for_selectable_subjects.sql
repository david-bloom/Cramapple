...
begin
  ...
  if not app.exam_pack_version_is_selectable(v_exam_pack_version_id) then
    raise exception 'home_session:subject_not_selectable'
      using errcode = '22023';
  end if;
...
comment on function app.start_home_learning_session_for_user(uuid, integer, text) is
  'Starts or resumes the caller selected-subject Home session. Active sessions for other subjects are preserved; Home entry requires a selectable subject, not Quick Start content eligibility.';

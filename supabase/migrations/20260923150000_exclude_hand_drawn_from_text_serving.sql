-- TASK-0038 Phase 1 — close an accidental serving-gate leak.
--
-- select_practice_frqs / select_unit_gated_practice_items are the two real
-- content selectors the live student-facing /session path calls. Neither
-- ever filtered on prompt_json->>'hand_drawn', so a published, targeted_drill
-- hand-drawn item (still ai_provisional_unapproved, evaluator_strategy
-- 'human_shadow') was structurally eligible to be served through the normal
-- text-answer FRQ path. The frontend has no way to render a hand-drawn
-- capture UI from that path (it only ever produces kind "mcq"/"short_frq"/
-- "long_frq"), so a served item would show a plain text box for a prompt
-- that expects a drawn graph. Zero real attempts have ever hit this in
-- Production -- this closes the gap before one does, not in response to an
-- incident.
--
-- Hand-drawn items remain servable through the dedicated attach_capture /
-- CaptureItem pipeline (TASK-0025, TASK-0038 Phase 3); this migration only
-- removes them from the two generic text-answer selectors.

-- FF-1 — make AP Biology's published MCQs reachable on the existing
-- targeted-drill practice path without assigning FRQ-only practice_format
-- semantics to MCQ content or changing any other subject's selector.
--
-- This selector is deliberately service-role only. student-session-items
-- verifies session ownership and routes only Biology targeted-drill sessions
-- here; all other subjects and formats continue to use
-- public.select_practice_frqs unchanged.

create or replace function app.select_biology_practice_items(
...
comment on function app.select_biology_practice_items(uuid, text, uuid, integer) is
  'FF-1 service-only Biology targeted-drill selector: proportional, session-stable published FRQ/MCQ mix with an answer-free projection.';

revoke execute on function app.select_biology_practice_items(uuid, text, uuid, integer)
  from public, anon, authenticated;
grant execute on function app.select_biology_practice_items(uuid, text, uuid, integer)
  to service_role;
